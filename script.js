document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateDisplay() {
    if (shuffledNumbers.length > 0) {
      ...
      positionDiv.textContent = `${currentIndex + 1} / ${
  ...
  shuffleButton.addEventListener("click", () => {
    ...
    if (isNaN(maxNumber) || maxNumber < 1) {
      resetDisplay();
      if (isNaN(maxNumber)) {
        positionDiv.textContent = "Por favor introduzca un número";
        return;
      } else if (maxNumber < 1) {
        positionDiv.textContent =
          "Por favor introduzca un número más grande que 0.";
        return;
      }
    }
  ...
  nextButton.addEventListener("click", () => {
    if (
      ...
      currentIndex < shuffledNumbers.length - 1
    ) {
      currentIndex++;
      updateDisplay();
    }
  ...
  maxNumberInput.addEventListener("input", () => {
    if (
      maxNumberInput.value.trim() === "" ||
      isNaN(parseInt(maxNumberInput.value, 10))
    ) {
      resetDisplay();
    }
  });
